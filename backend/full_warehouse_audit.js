...
async function runAudit() {
  ...
  try {
    ...
    console.log('\nWaiting 1.5s for locks to clear...');
    await delay(1500);

    console.log('Sending inward scan request to /api/scanner/auto-store...');
    ...
    const jsonInward = resInward.status === 200 ? await resInward.json() : await resInward.text();
    ...
    if (qrAfterInward && qrAfterInward.status === 'unused') {
      results.qrSync.status = 'PASS';
      results.qrSync.details = 'status successfully remained unused.';
    } else {
      results.qrSync.details = `status changed to '${qrAfterInward ? qrAfterInward.status : 'N/A'}'.`;
    }
    ...
    const jsonOutward = resOutward.status === 200 ? await resOutward.json() : await resOutward.text();
    ...
    console.log('Rack state AFTER outward scan:', rackAfterOutward ? {
    ...
    } : 'DELETED (Quantity became 0)');

    const outwardRackPass = !rackAfterOutward || parseFloat(rackAfterOutward.current_capacity) < parseFloat(rackAfterInward.current_capacity);
    if (inwardRackPass && outwardRackPass) {
      results.rackSync.status = 'PASS';
      results.rackSync.details = `Rack occupancy correctly synced. Inward: ${rackAfterInward.occupancy_percentage}%, Outward: ${rackAfterOutward ? rackAfterOutward.occupancy_percentage : 0.00}% (Rack deleted on depletion).`;
    } else {
    ...
    const resLowIn = await fetch(`${BASE_URL}/scanner/auto-store`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': authHeader
    ...
    console.log(`Low Inward HTTP Status: ${resLowIn.status}`);
    const jsonLowIn = await resLowIn.json();
    console.log('Low Inward Response:', jsonLowIn);
    ...
    const resLowOut = await fetch(`${BASE_URL}/scanner/outward`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': authHeader
    ...
    console.log(`Low Outward HTTP Status: ${resLowOut.status}`);
    const jsonLowOut = await resLowOut.json();
    console.log('Low Outward Response:', jsonLowOut);
